...
class Schedule:
    ...
    def __init__(self, parent_order, child_window: int, pattern: list) -> None:
        """
        methode that defines a time schedule per parent order which is called
        by the order itself

        :param parent_order:
            ParentOrder, object of the parent order which asks for a schedule
        :param child_window:
        ...
        while time_unreached:
            if self.start_time \
                ...
                offset_time = self.start_time + pd.DateOffset(
                    minutes=self.child_window * count)
            else:
                offset_time = \
                    parent_order.NEXT_MARKET_START \
                    + ((self.start_time
                        + pd.DateOffset(minutes=self.child_window * count))
                       - parent_order.MARKET_END)
        ...
        if vol_left > 0:
            ...
            volumes[np.arange(len(time_stamps) + vol_left,
                              len(time_stamps))] -= 1
    ...
    def stay_scheduled(self, timestamp: pd.Timestamp) -> None:
        ...
        if self.parent_order.active:
            orders_to_fill = self.scheduling[
                self.scheduling['timestamp'] < timestamp]
            if orders_to_fill['volume'].sum() > 0:
                self.parent_order.child_orders.append(
                    self.parent_order.agent.market_interface.submit_order(
...
class ParentOrder:
    """
    This is the Parent Order class wich is used to simulate
    incoming parent orders and manege corresponding child orders
    """
    def __init__(self, ts: pd.Timestamp, volume_range: list, stock: str,
                 avg_vol: float, time_window_length: int,
                 agent, child_window: int, pattern: list) -> None:
        """
        ...
        :param volume_range:
            list, containing percentage values of daily volume representing the
            interval from which the order size is sampled
        :param stock:
            string, stock symbol of the stock ordered
        :param avg_vol:
            float, average volume trade volume of the stock
        :param time_window_length:
            int, time in which the order should be filled in hours
        :param agent:
            agent, agent object
        :param child_window:
    ...
    def execution(self, exec_trade: Trade) -> None:
        """
        ...
        if self.volume_left <= 0:
            ...
            self.agent.parent_orders[self.symbol].append(ParentOrder(
                exec_trade.timestamp, self.agent.vol_range, self.symbol,
                self.agent.stock_mean_vol[self.symbol], self.agent.time_window,
                self.agent, child_window, pattern))
    ...
    def actualize_vwap(self, trades_state: pd.Series) -> None:
        """
        method to track the market vwap in the same time window as
        the parent order
