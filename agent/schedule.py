...
class Schedule():
    ...
    timing of it
    '''
    def __init__(self, parent_order:Parent_order, child_window:int, pattern:list) -> None:
        '''
        ...
        self.meta_schedule_time = [self.start_time+pd.DateOffset(hours=i) for i in range(1,diff)]
    ...
    def stay_scheduled(self, market_state:MarketState, timestamp:pd.Timestamp) -> dict:
        '''
        ...
        if np.all(self.meta_schedule[self.meta_schedule.index<timestamp]):
            limit = market_state.best_ask if self.parent_order.side == 'buy' else market_state.best_bid
        else:
            limit = None
