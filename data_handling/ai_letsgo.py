...
import keras
from data_handling.handle_data import get_file_names, \
    get_raw_book, get_midpoints
...
def transform_minutes(book_states):
    in_time = True
    timestamp = pd.Timestamp(2021, book_states.index[0].month,
                             book_states.index[0].day, 8, 0)
    ...
    while in_time:
        ...
        timestamp = tmp_time
        ind += 1
        if tmp_time > pd.Timestamp(2021, book_states.index[0].month,
                                   book_states.index[0].day, 16, 29):
            in_time = False
    return minutes
...
def build_training(mins):
    window = 20
    label_range = 3
    x_vals = np.zeros((mins.shape[0]-window-label_range,
                       window, mins.shape[1]))
    y_vals = np.zeros((mins.shape[0]-window-label_range, 1))
    for i in range(1, x_vals.shape[0]):
        x_vals[i-1, :, :] = mins[i:i+window, :].copy() \
                            / mins[i-1:i-1+window, :].copy() - 1
...
def run_lstm_training():
    stock_list = ['Adidas', 'Allianz', 'BASF', 'Bayer', 'BMW', 'Continental', 
                  ...
                  'DeutscheBörse']
    # TODO wäre hier evtl. eine globale Variable sinnvoll?
    ...
    model.add(Dense(1, activation='sigmoid',
                    kernel_regularizer=keras.regularizers.L1L2(l1=1e-8,
                                                               l2=1e-8),
