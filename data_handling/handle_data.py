...
def create_volume_distribution(stock_list, dir_list_t, dir_list_b):
    ...
    for stock in tqdm(stock_list):
        ...
        for file_t, file_b in zip(files_t, files_b):
            path_t = u'C:/Users/Lucas/Downloads/archive/_shared_storage/' \
                     u'read_only/efn2_backtesting/trades/' + file_t
            path_b = u'C:/Users/Lucas/Downloads/archive/_shared_storage/' \
                     u'read_only/efn2_backtesting/book/' + file_b
            ...
            if trade is not None:
                ...
                for i in range(trade.shape[0]):
                    ...
                    edges.loc[ind, 'Price'] = np.abs(trade.loc[ind, 'Price']
                                                     - mid[timestamp])
...
def create_volume_summary(stock_list, dir_list):
    ...
    for stock in tqdm(stock_list):
        ...
        hour_avg = None
        for file in files:
            with open(u'C:/Users/Lucas/Downloads/test_trade_download/trades/'
                      + file, 'r') as json_file:
                data = json.load(json_file)
            if len(data) > 0:
                data_df = pd.DataFrame(data)
                data_df['TIMESTAMP_UTC'] = \
                    pd.to_datetime(data_df['TIMESTAMP_UTC'])
                data_df['hours'] = data_df['TIMESTAMP_UTC'].dt.hour
                data_df['Volume_agr'] = \
                    data_df.apply(lambda row: sum(row['Volume']), axis=1)
                summary = \
                    data_df[['hours', 'Volume_agr']].groupby(['hours']).sum()
                if hour_avg is None:
    ...
    for stock in tqdm(stock_list):
        ...
        for file in files:
            ...
            if len(data) > 0:
                ...
                summary = \
                    data_df[['hours',
                             'Volume', 'Multiply']].groupby(['hours']).sum()
                ...
                for ind in range(summary.shape[0]):
                    ...
                    std_agr[ind] = \
                        ((data_df.loc[data_df['hours'] == hour, 'Price']
                          - summary.loc[hour, 'Price_avg'])**2).sum() \
                        / summary.loc[hour, 'Price_avg']
                if hour_avg is None:
                    hour_avg = std_agr
                else:
                    hour_avg += std_agr
...
if __name__ == '__main__':
    stock_list = ['Adidas', 'Allianz', 'BASF', 'Bayer', 'BMW', 'Continental',
                  'Covestro', 'Covestro', 'Daimler',
                  'DeutscheBank', 'DeutscheBörse']
