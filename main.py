...
from agent.parentorder import ParentOrder
...
def determinate_price(market_state, order: Order) -> list:
    level = '1'
    side = 'Bid' if order.side == 'buy' else 'Ask'
...
class Agent(BaseAgent):
    ...
    def __init__(self, name, identifiers):
        ...
        The backtest iterates over a set of sources and alerts the
        trading agent whenever a source is updated. These updates are conveyed
        through method calls to, all of which you are expected
    ...
    def on_time(self, timestamp: pd.Timestamp,
                timestamp_next: pd.Timestamp) -> None:
        """
        ...
        if not self.orders_initialized:
            ...
            for stock in self.parent_orders:
                self.parent_orders[stock].append(
                    ParentOrder(timestamp, self.vol_range, stock,
                                self.stock_mean_vol[stock], self.time_window,
                                self, self.child_window, self.volume_pattern))
    ...
    def set_parent_order_status(self, timestamp: pd.Timestamp) -> None:
        """
        method to check at a moment in time if orders are active
        depending on their time_window
    ...
    def update_limit_order(self, market_id, timestamp, market_state) -> None:
        ...
        for order in orders:
            if order.limit is not None:
                limit_order = True
                if update_needed(market_state, order, timestamp):
                    ...
                    order.parent.child_orders.append(
                        self.market_interface.submit_order(
                            ...
                            quantity=order.parent.schedule.get_outstanding(
                                timestamp),
                            limit=determinate_price(market_state, order),
                            parent=order.parent
                            ))
