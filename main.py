...
class Agent(BaseAgent):
    ...
    def __init__(self, name, identifiers, *args, **kwargs):
        """
        Trading agent implementation.

        The backtest iterates over a set of sources and alerts the trading
        agent whenever a source is updated. These updates are conveyed through
        method calls to, all of which you are expected to implement yourself:
        ...
        # TODO move to decision support
        self.stock_hourly_vol = \
            pd.read_csv('./agent/resources/'
    ...
    def on_quote(self, market_id: str, book_state: pd.Series) -> None:
        """
        ...
        if self.orders_initialized:
            if self.parent_orders[market_id][-1].active:
                self.update_limit_order(
                    market_id, book_state['TIMESTAMP_UTC'], book_state)

        return None

    def on_trade(self, market_id: str, trades_state: pd.Series) -> None:
        """
    ...
    def on_time(self, timestamp: pd.Timestamp, timestamp_next: pd.Timestamp):
        ...
        if not self.orders_initialized:
            ...
            for stock in self.parent_orders:
                self.parent_orders[stock].append(Parent_order(
                    timestamp, self.vol_range, stock,
                    self.stock_mean_vol[stock], self.time_window, self,
                    self.child_window, self.volume_pattern))
            self.orders_initialized = True

        # check every minute if orders are outdated
        # check if they are in their schedule         
        if self.check_status is None or self.check_status \
                + pd.DateOffset(minutes=1) < timestamp:
            self.check_status = timestamp
            print(timestamp)
            self.set_parent_order_status(timestamp)
            self.stay_scheduled(timestamp)

    def set_parent_order_status(self, timestamp: pd.Timestamp) -> None:
        """
        method to check at a moment in time if orders are active depending
        on their time_window

        :params timestamp:
            pd.Timestamp, timestamp of the moment the method is called
        """
        for stock in self.parent_orders:
            if self.parent_orders[stock][-1].active is False \
                    and self.parent_orders[stock][-1].time_window[0] \
                    < timestamp \
                    and self.parent_orders[stock][-1].volume_left > 0:
    ...
    def stay_scheduled(self, timestamp: pd.Timestamp) -> None:
        """
        ...
        """
        for stock in self.parent_orders:
            market_order = self.parent_orders[
                stock][-1].schedule.stay_scheduled(timestamp)
            if market_order is not None:
                self.parent_orders[stock][-1].child_orders.append(
    ...
    def update_needed(self, market_state, order: Order, timestamp) -> bool:
        trigger = False
        level = '1'
        side = 'Bid' if order.side == 'buy' else 'Ask'
        if market_state['L'+level+'-'+side+'Price'] != order.limit \
                and order.parent.schedule.get_outstanding(timestamp) > 0:
            trigger = True

        return trigger

    def determinate_price(self, market_state,
                          order: Union[Order, Parent_order]) -> List:
        level = '1'
        side = 'Bid' if order.side == 'buy' else 'Ask'
        return market_state['L'+level+'-'+side+'Price']

    def update_limit_order(self, market_id, timestamp, market_state) -> None:
        """
        method to send limit orders to the market, dependent on the
        agent strategy
        """
        orders = self.market_interface.get_filtered_orders(
        ...
        for order in orders:
            if order.limit is not None:
                ...
                if self.update_needed(market_state, order, timestamp):
                    ...
                    order.parent.child_orders.append(
                        self.market_interface.submit_order(
                            market_id=market_id,
                            side=order.side,
                            quantity=order.parent.schedule.get_outstanding(
                                timestamp),
...
if __name__ == "__main__":
    ...
    agent = Agent(name="test_agent", identifiers=identifier_list)
    ...
    backtest.run_episode_list(identifier_list=identifier_list,
                              episode_list=[("2021-01-04T08:00:00",
                                             "2021-01-04T08:15:00",
                                             "2021-01-04T09:30:00")]
                              )
